...
| 创建多余的"归档 PR" | PR 合并后迭代即完成，不需要额外归档 PR |

## 经验教训

### 不要创建多余的"归档 PR"

- **问题**：PR 合并后，又创建了一个单独的分支来更新流程状态和重命名迭代目录
- **原因**：误以为这些状态变更需要单独提 PR
- **正确做法**：PR 合并 = 迭代完成。流程状态更新和目录重命名是本地管理操作，不需要单独提 PR
- **预防**：记住 P15 的核心是"创建 PR → 合并 → 清理分支"，合并后迭代就完成了
